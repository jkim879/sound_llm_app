...
def extract_keywords(text, model_option):
    response = client.chat.completions.create(
        model=model_option,
        messages=[
            {"role": "system", "content": "기업신용평가회사 '이크레더블'이 고객에게 전화를 건 대화 내용입니다. 이 대화의 가장 핵심 키워드 추출. 답변은 부가 설명 없이 핵심 키워드만 추출."},
    ...
    for chunk in chunks:
        if chunk.strip():
            response = client.chat.completions.create(
                model=model_option,
                messages=[
                    {"role": "system", "content": "기업신용평가회사 '이크레더블'이 고객에게 전화를 건 대화 내용입니다. 전화를 받은 사람의 텍스트의 감정만 분석하여 '긍정', '중립', '부정' 중 하나로 분류하고, -1(매우 부정)에서 1(매우 긍정) 사이의 점수를 함께 제시해주세요. 형식: [감정];[점수]"},
...
def classify_topics(text, model_option):
    response = client.chat.completions.create(
        model=model_option,
        messages=[
            {"role": "system", "content": "기업신용평가회사 '이크레더블'이 고객에게 전화를 건 대화 내용입니다. 텍스트 주제 분류. 답변은 부가설명 없이 주제만 추출."},
            {"role": "user", "content": text}
        ],
        max_tokens=100
    )
                ...
                try:
                    text_response = client.chat.completions.create(
                        ...
                        messages=[
                            ...
                            {"role": "user", "content": f"기업신용평가회사 '이크레더블'이 고객에게 전화를 건 대화 내용입니다. 다음 텍스트에서 전화를 받은 사람의 감정만 분석해주세요: {transcribed_text}. 대답은 반드시 요약 형식으로 답변."}
                        ],
                        max_tokens=100,
                        temperature=0.5
                    )
